...
- 47 已验证：白名单发布 `whitelist-7a066b2e3c36d800fa85cbf5`，运行源码 414/414 一致，容器内 USD 720 RMB → USD 100 换算及过期汇率拒绝探针通过，API 容器正常，公网 `/api/health` 200，发布锁与 recovery 状态均为 clear。
...
- 未验证项：两条旧 Finance E2E 需要后续按待审核队列语义更新；本阶段没有用生产业务数据执行写接口。
- 发布状态：`已发布 47，release whitelist-7a066b2e3c36d800fa85cbf5`
- 稳定附件：无
- 准确下一步：第二阶段先补水单 Repository port 与两适配器契约测试，再把匹配申请编排从巨型 Repository 移入 application service；保持路由、schema、权限和审核队列不变。
