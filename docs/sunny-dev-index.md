...
1. 用户明确指定 `docs/tasks/<任务卡>` 时，代码会话先按 `docs/tasks/_acceptance.md` 检查准入；用户在当前会话明确要求实施且范围清楚时，也可直接执行非任务卡需求。
2. 代码会话从 `docs/dev-now/_template.md` 创建并只维护自己的 `docs/dev-now/<session-slug>.md`；输入来源填写任务卡路径或“当前会话明确请求”，`docs/dev-now.md` 只读，不得自动改写。
3. 复杂或仍需业务决策的新需求优先由规划会话按 `docs/tasks/_template.md` 输出任务卡；范围明确的修复、实现和局部改动无需为了形式补卡。
4. A-F 历史任务继续打开对应 `docs/slices/<任务卡>`，按该卡的“开工前现状审计清单”审计现有页面、接口、表、状态、权限、日志和测试。
5. 只做任务卡允许的最小补差；并发会话优先使用独立 worktree 和独立 `codex/<session-slug>` 分支。
6. 完工后先把验证结论写回自己的会话状态文件；`.codex-state.md` 只由主推进会话或用户明确授权的会话追加高信号完成记录，并维持最近 7 天归档规则。
