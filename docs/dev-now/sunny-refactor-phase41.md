...
- 固定样本：对一个容器 inspect JSON，当 `.Image=sha256:config`且 `.ImageManifestDescriptor.digest=sha256:manifest` 时，发布状态、audit 与运行清单统一取 manifest digest；当 descriptor 缺失时回退 `.Image`。运行清单升级为 v3，直接从容器 inspect 保存 manifest/config 双重证据，不再对可能已被清理的 image config 对象执行 `docker image inspect`。
