...
export function PriceBookManagementToolbar({
  ...
  return (
    <Space>
      {classified && can('pricing:price-books:import') ? <Select
        aria-label="选择代理简称"
        showSearch
        placeholder="选择代理简称"
        value={importAgentId}
      ...
      {can('pricing:price-books:import') ? <Button htmlType="button" size="small" icon={<RefreshCw size={14} />} loading={importHistoryLoading} onClick={onLoadImportHistory}>导入记录</Button> : null}
      {can('pricing:price-books:health') ? <Button htmlType="button" size="small" icon={<Settings size={14} />} loading={syncHealthLoading} onClick={onOpenSyncHealth}>同步体检</Button> : null}
      {classified && can('pricing:price-books:import') ? <Button
        htmlType="button"
        size="small"
      ...
      {classified && can('pricing:price-books:import') ? <input
        ref={fileInputRef}
      ...
      /> : null}
      {can('pricing:price-books:export') ? <Button htmlType="button" size="small" icon={<Download size={14} />} disabled={selectedCount !== 1} title="下载导入时保留的原始 xls/xlsx 价格表" onClick={onDownload}>下载价格表</Button> : null}
      {classified && can('pricing:price-books:update') ? <Button htmlType="button" size="small" disabled={selectedCount !== 1} onClick={onEditRemark}>编辑默认备注</Button> : null}
