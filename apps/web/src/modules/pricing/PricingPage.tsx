...
import { Alert, Button, Card, Checkbox, Col, Form, Input, InputNumber, Modal, Popconfirm, Row, Select, Space, Table, Tag, Typography } from 'antd';
...
import type { AgentMarkupListQuery, AgentMarkupListResponse, AgentMarkupMetrics, AgentMarkupSummary, AgentMarkupType, LegacyPricingMetaResponse, LegacyPricingModule, LegacyPricingQuoteRequest, LegacyPricingQuoteResponse, LegacyPricingRecommendation, PriceBookImportInput, PriceBookSummary, PriceLookupRecommendation, PriceLookupResponse, QuoteSourceType, StaffRoleKey } from '@siyuan/shared';
...
type LegacyLookupFormValues = PriceLookupFormValues & Partial<Omit<LegacyPricingQuoteRequest, 'module'>>;
const pricingManagerRoles = new Set<StaffRoleKey>(['ADMIN', 'UG_MARKET']);

const legacyPricingModules: Array<{ key: LegacyPricingModule; label: string }> = [
  { key: 'amazon', label: '亚马逊查询' },
  { key: 'inquiry', label: '欧洲海运超大件查询' },
...
function formatLegacyModuleCounts(counts?: PriceBookSummary['legacyModuleCounts']) {
  ...
  const text = legacyPricingModules
    .map((item) => `${item.label.replace('查询', '')} ${counts[item.key] ?? 0} 条`)
    .join(' / ');
  return text || '未解析';
}
...
export function PricingPage({
  ...
  const priceBookMarkupRules = useMemo<MarkupDisplayRule[]>(() => {
    if (!priceBooks.length) return [];
    return priceBooks.map((book) => {
      const rows = priceRows.filter((row) => row.priceBookId === book.id);
      const agents = Array.from(new Set(rows.map((row) => row.agentName).filter(Boolean)));
      const relatedRules = markupDetailRules.filter((rule) => agents.includes(rule.agentName));
      const primary = relatedRules.find((rule) => !rule.channelName && !rule.realChannelName && !rule.destinationCountry) ?? relatedRules[0];
      const latestUpdatedAt = relatedRules.map((rule) => rule.updatedAt).filter(Boolean).sort().at(-1);
      return {
        ...(primary ?? {
          markupPerKg: 0.5,
        ...
        id: `price-book:${book.id}`,
        agentName: book.fileName,
        priceBookId: book.id,
        priceBookFileName: book.fileName,
        isPriceBookGroup: true,
        channelName: undefined,
        realChannelName: undefined,
        destinationCountry: agents.join(' / ') || undefined,
        ruleCount: agents.length || relatedRules.length || 0,
        hitCount: rows.length,
        updatedAt: latestUpdatedAt ?? book.importedAt,
        enabled: relatedRules.length ? relatedRules.some((rule) => rule.enabled) : true
      };
    });
  }, [markupDetailRules, priceBooks, priceRows]);
  const markupDisplayRows: MarkupDisplayRule[] = priceBookMarkupRules.length ? priceBookMarkupRules : markupRules;
  ...
  const selectedPriceBook = selectedPriceBookIds.length === 1 ? priceBooks.find((book) => book.id === selectedPriceBookIds[0]) ?? null : null;
  ...
  const selectedMarkupChannelRows = activeMarkupChannelRule
    ? activeMarkupChannelRule.isPriceBookGroup && activeMarkupChannelRule.priceBookId
      ? priceRows.filter((row) => row.priceBookId === activeMarkupChannelRule.priceBookId)
  ...
  const canViewMarkupDetails = pricingManagerRoles.has(role);
  ...
  const canRunLookup = (() => {
    ...
    if (legacyModule === 'europeExpress') return Boolean(destinationCountryValue?.trim()) && Boolean(channelValue?.trim());
  ...
  const highlightedQuote = useMemo(() => {
    if (recommendationFilter === 'FASTEST') {
      return lookupResult?.fastestRecommendations[0] ?? null;
    }
  ...
  useEffect(() => {
    ...
    apiClient.legacyPricingMeta()
      .then((response) => {
        if (alive) setLegacyMeta(response);
      })
      .catch(() => undefined);
    return () => {
      alive = false;
  ...
  function resolveConcreteMarkupRule(rule: MarkupDisplayRule) {
    if (rule.isPriceBookGroup) return rule;
  ...
  async function resolveAgentRuleIds(rule: MarkupDisplayRule) {
    if (rule.isPriceBookGroup) return [];
  ...
  async function handlePriceFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setSelectedPriceBookIds([imported.book.id]);
      ...
      onNotice(`已导入价格表 ${file.name}，新增 ${imported.rows.length} 条代理成本价，亮崽模块：${formatLegacyModuleCounts(imported.book.legacyModuleCounts)}，同步 ${createdMarkupRules.length} 条加价规则`);
      event.target.value = '';
    } catch (error) {
  ...
  async function deleteSelectedPriceBooks() {
    const selectedBooks = priceBooks.filter((book) => selectedPriceBookIds.includes(book.id));
    ...
    try {
      ...
      onNotice(selectedBooks.length === 1 ? `已删除价格表 ${selectedBooks[0].fileName}` : `已删除 ${selectedBooks.length} 张价格表`);
    } catch (error) {
  ...
  async function runLookup() {
    try {
      ...
      const tierWeight = Number(String(values.tier ?? '').match(/\d+(?:\.\d+)?/)?.[0] ?? 0);
      const requestChargeableWeightKg = Number(values.chargeableWeightKg) || calculatedChargeableWeight || tierWeight || 0;
      ...
      if (legacyModule === 'europeExpress' && !values.channel?.trim()) {
        onNotice('请先选择渠道');
        return;
      }
      ...
      if (legacyModule === 'inquiry' && isPostalCodeRequired(destinationCountry ?? '') && !postalCode) {
        lookupForm.setFields([{ name: 'postalCode', errors: ['当前目的地需要填写邮编'] }]);
        onNotice('当前目的地需要填写邮编');
        return;
      }
      const legacy = await apiClient.quoteLegacyPricing({
        module: legacyModule,
        ...values,
        amazonCode,
      ...
      onNotice(legacy.selected
        ? `${legacy.selected.agentName} ${legacy.selected.destinationCountry ?? destinationCountry} ${legacy.selected.chargeableWeightKg}kg 报价 ${formatCurrency(legacy.selected.salesTotal)}`
  ...
  function renderLegacyLookupFields() {
    const agentOptions = [
      { value: '', label: '全部代理' },
      ...(legacyMeta?.agents ?? []).map((value) => ({ value, label: value }))
    ];
    const originOptions = [
      { value: '', label: '全部出货仓' },
      ...(legacyMeta?.origins ?? []).map((value) => ({ value, label: value }))
    ];
    const tierOptions = [
          ...
          <section className="pricing-form-block">
            ...
            <div className="pricing-form-grid pricing-form-grid-amazon">
              <Form.Item name="amazonCode" label="亚马逊仓库代码" rules={[{ required: true, message: '请输入仓库代码' }]}>
                <Input placeholder="FTW5" />
              </Form.Item>
              <Form.Item name="tier" label="重量段">
                <Select options={tierOptions} />
              </Form.Item>
              <Form.Item name="agentName" label="代理">
                <Select showSearch options={agentOptions} />
              </Form.Item>
              <Form.Item name="origin" label="出货仓">
                <Select showSearch options={originOptions} />
              </Form.Item>
              <Form.Item name="destinationCountry" label="国家/地区关键词">
                <Input placeholder="国家/地区关键词" />
              </Form.Item>
              <Form.Item name="channel" label="渠道关键词">
                <Input placeholder="渠道关键词" />
              </Form.Item>
              <Form.Item name="packageInfo" label="源文件关键词">
                <Input placeholder="源文件关键词" />
              </Form.Item>
              <Form.Item name="onlyQuotable" label="只看可报价" valuePropName="checked">
                <Checkbox>只看可报价</Checkbox>
              </Form.Item>
              <Form.Item name="actualWeightKg" label="实重 KG">
                <InputNumber className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="如 500" />
              </Form.Item>
              <Form.Item name="volumeCbm" label="方数 CBM">
                <InputNumber aria-label="方数" className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="如 1" />
              </Form.Item>
              ...
              <Form.Item name="channel" label="渠道">
                <Select
                  options={[
                    { value: '', label: '全部渠道' },
                    { value: '海运', label: '海运' },
                    { value: '卡派', label: '卡派/超大件' },
                    { value: '快递', label: '快递' }
                  ]}
                />
              </Form.Item>
              <Form.Item name="actualWeightKg" label="实际重量 KG">
                <InputNumber className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="没有可不填" />
              </Form.Item>
              <Form.Item name="volumeCbm" label="方数 CBM" rules={[{ required: true, message: '请输入方数' }]}>
                <InputNumber aria-label="方数" className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="5" />
              </Form.Item>
            </div>
          </section>
          <section className="pricing-form-block">
            <Text strong className="pricing-form-block-title">有详细尺寸再填</Text>
            <div className="pricing-form-grid pricing-form-grid-size">
              <Form.Item name="lengthCm" label="长 cm"><InputNumber min={0} precision={2} placeholder="可不填" /></Form.Item>
              <Form.Item name="widthCm" label="宽 cm"><InputNumber min={0} precision={2} placeholder="可不填" /></Form.Item>
              <Form.Item name="heightCm" label="高 cm"><InputNumber min={0} precision={2} placeholder="可不填" /></Form.Item>
              <Form.Item name="packageCount" label="件数"><InputNumber min={1} precision={0} /></Form.Item>
              <Form.Item name="unitActualWeightKg" label="单件实重 KG"><InputNumber min={0} precision={3} placeholder="不知道可不填" /></Form.Item>
            </div>
          </section>
        </>
      );
        ...
        <section className="pricing-form-block">
          ...
          <div className="pricing-form-grid pricing-form-grid-express">
            ...
            <Form.Item name="chargeableWeightKg" label="计费重量 KG">
              <InputNumber className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="如 80、120、1000；不填则按最低单价排序" />
            </Form.Item>
          </div>
        </section>
      );
      ...
      <section className="pricing-form-block">
        ...
        <div className="pricing-form-grid pricing-form-grid-south-africa">
          <Form.Item name="productName" label="品名/明细关键词" rules={[{ required: true, message: '请输入品名或关键词' }]}>
            <Input placeholder="衣服" />
          </Form.Item>
          <Form.Item name="tier" label="指定物料类别">
            <Select showSearch options={tierOptions} />
          </Form.Item>
          <Form.Item name="volumeCbm" label="体积 CBM" rules={[{ required: true, message: '请输入体积' }]}>
            <InputNumber aria-label="方数" className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="1" />
          </Form.Item>
          <Form.Item name="actualWeightKg" label="重量 KG">
            <InputNumber className="pricing-measure-input" controls={false} min={0} precision={3} placeholder="可不填" />
          </Form.Item>
          <Form.Item name="packageInfo" label="备注/申报信息" className="pricing-field-span-2">
            <Input.TextArea rows={3} placeholder="可填客户描述，系统会一起匹配关键词" />
          </Form.Item>
        </div>
      </section>
    );
      ...
      <ModuleSubWorkspace items={pricingSubItems} activeKey={activePricingSection} onChange={setActivePricingSection}>
        ...
        <Col xs={24}>
            ...
            <Space wrap>
              {legacyPricingModules.map((item) => (
                <Button
                  ...
                  onClick={() => changeLegacyModule(item.key)}
                >
                  {item.label}
                </Button>
              ))}
          ...
          >
            ...
            >
              <div className={`pricing-calculator-grid pricing-calculator-grid-${legacyModule}`}>
                <div className="pricing-calculator-left">
                  ...
                  <div className="pricing-legacy-action-row">
                    ...
                    >
                      {legacyModule === 'amazon' ? '查询比价' : legacyModule === 'inquiry' ? '查询最低报价' : '查询价格'}
                    </Button>
                    <Button size="large" onClick={resetLookupResult}>清空</Button>
                  </div>
                </div>
              </div>
            </Form>
            ...
            {lookupResult ? (
              <div className="pricing-workbench">
                ...
                <div className="pricing-result-toolbar">
                  <Text type="secondary">共匹配 {lookupResult.recommendations.length} 条渠道</Text>
                  <Space wrap>
                        {[
                          ['ALL', '全部'],
                          ['RECOMMENDED', '推荐'],
                          ...
                          <Button
                            key={key}
                            size="small"
                            disabled={key === 'FASTEST' && !lookupResult.fastestRecommendations.length}
              ...
              <div className="pricing-empty-result">
                ...
                <div>
                  <Text strong>已按当前查价模块输出报价</Text>
                  <div><Text type="secondary">结果见下方模块报价表。</Text></div>
                </div>
              </div>
            ) : (
              <>
          ...
          {legacyResult ? (
            <Card className="module-grid pricing-legacy-result-card" title={`${legacyPricingModules.find((item) => item.key === legacyResult.module)?.label ?? '亮崽兼容'} · 业务报价`}>
              <Row gutter={[12, 12]}>
                {(legacyResult.cheapestRecommendations.length ? legacyResult.cheapestRecommendations : legacyResult.recommendations.slice(0, 3)).map((item, index) => (
                  <Col xs={24} md={8} key={item.id}>
                    <button type="button" className="pricing-legacy-quote-card" onClick={() => setSelectedLegacyRecommendation(item)}>
                      ...
                      <span>{canViewMarkupDetails ? `${item.agentName} / 成本 ${formatCurrency(item.costTotal)}` : item.agentName}</span>
                      <span>{item.weightSegmentLabel} / {formatKgCurrencyRate(item.salesUnitPrice)}/kg</span>
                      {item.productSurchargeRemark || item.specialRemark || item.remark ? <Tag color="orange">有备注</Tag> : null}
                    </button>
                  </Col>
                ))}
              </Row>
              <Table
                ...
                scroll={{ x: canViewMarkupDetails ? 1180 : 980 }}
                columns={[
                  { title: '代理', dataIndex: 'agentName', width: 120 },
                  ...
                  { title: '单价', dataIndex: 'salesUnitPrice', width: 110, render: (value) => `${formatKgCurrencyRate(value)}/kg` },
                  { title: '总价', dataIndex: 'salesTotal', width: 120, render: (value) => <Text strong>{formatCurrency(value)}</Text> },
                  ...(canViewMarkupDetails ? [{ title: '毛利', dataIndex: 'grossProfit', width: 100, render: (value?: number) => <Text className="pricing-profit">{value === undefined ? '-' : formatCurrency(value)}</Text> }] : []),
                  { title: '备注', width: 100, render: (_value, record) => record.productSurchargeRemark || record.specialRemark || record.remark ? <Button type="link" size="small" onClick={() => setSelectedLegacyRecommendation(record)}>查看</Button> : <Text type="secondary">-</Text> }
                ]}
              />
            </Card>
          ) : null}
              ...
              {canViewMarkupDetails ? (
                <div className="pricing-result-grid pricing-admin-only">
                  ...
                  <div className="pricing-result-item">
                    ...
                    <Text strong>{selectedPriceRecommendation.price.costPerKg === undefined ? '-' : `${selectedPriceRecommendation.price.currency} ${formatKgRate(selectedPriceRecommendation.price.costPerKg)}/kg`}</Text>
                  </div>
                  <div className="pricing-result-item">
                    <Text type="secondary">成本合计</Text>
                    <Text strong>{selectedPriceRecommendation.totalCost === undefined ? '-' : formatCurrency(selectedPriceRecommendation.totalCost)}</Text>
                  </div>
                  <div className="pricing-result-item">
                    <Text type="secondary">毛利</Text>
                    <Text strong>{selectedPriceRecommendation.grossProfit === undefined ? '-' : formatCurrency(selectedPriceRecommendation.grossProfit)}</Text>
                  </div>
                </div>
              ) : null}
        ...
        <Modal
          ...
          destroyOnHidden
          footer={<Button type="primary" onClick={() => setSelectedLegacyRecommendation(null)}>关闭</Button>}
          ...
          {selectedLegacyRecommendation ? (
            <Space direction="vertical" size={14} className="full-width pricing-detail-modal">
              <div className="pricing-result-grid">
                ...
                <div className="pricing-result-item"><Text type="secondary">代理</Text><Text strong>{selectedLegacyRecommendation.agentName}</Text></div>
                ...
                <div className="pricing-result-item"><Text type="secondary">业务单价</Text><Text strong>{formatKgCurrencyRate(selectedLegacyRecommendation.salesUnitPrice)}/kg</Text></div>
              ...
              {canViewMarkupDetails ? (
                <div className="pricing-result-grid pricing-admin-only">
                  <div className="pricing-result-item"><Text type="secondary">成本单价</Text><Text strong>{formatKgCurrencyRate(selectedLegacyRecommendation.costUnitPrice)}/kg</Text></div>
                  <div className="pricing-result-item"><Text type="secondary">成本合计</Text><Text strong>{formatCurrency(selectedLegacyRecommendation.costTotal)}</Text></div>
                  <div className="pricing-result-item"><Text type="secondary">毛利</Text><Text strong>{selectedLegacyRecommendation.grossProfit === undefined ? '-' : formatCurrency(selectedLegacyRecommendation.grossProfit)}</Text></div>
                </div>
              ) : null}
              <div className="pricing-detail-note"><Text type="secondary">完整备注</Text><Text>{selectedLegacyRecommendation.remark || '暂无备注'}</Text></div>
              <div className="pricing-detail-note"><Text type="secondary">产品附加</Text><Text>{selectedLegacyRecommendation.productSurchargeRemark || '暂无产品附加说明'}</Text></div>
              <div className="pricing-detail-note"><Text type="secondary">特别说明/尺寸要求</Text><Text>{selectedLegacyRecommendation.specialRemark || '暂无特别说明/尺寸要求'}</Text></div>
            </Space>
          ) : null}
        </Modal>
          ...
          <Col xs={24}>
            <div className="pricing-markup-workbench">
              ...
              <Card
                ...
                extra={
                  <Space wrap>
                    ...
                    <Button size="small" icon={<FileInput size={14} />} onClick={() => onNotice('请使用规则模板上传入口导入')}>导入规则</Button>
                    ...
                    <Button size="small" disabled={selectedVisibleMarkupRuleIds.length !== 1 || selectedMarkupRuleIsPriceBookGroup} onClick={openEditMarkupRule}>修改</Button>
                    ...
                    >
                      <Button size="small" icon={<Power size={14} />} disabled={selectedVisibleMarkupRuleIds.length !== 1 || selectedMarkupRuleIsPriceBookGroup || selectedMarkupRule?.enabled === false}>停用</Button>
                    </Popconfirm>
                    <Popconfirm
                      title={`确认删除 ${selectedVisibleMarkupRuleIds.length} 条加价规则？`}
                      description="删除后不可恢复；已被历史报价引用时仅停用展示。"
                      okText="确认删除"
                      cancelText="取消"
                      okButtonProps={{ danger: true }}
                    ...
                    >
                      <Button size="small" danger icon={<Trash2 size={14} />} disabled={selectedVisibleMarkupRuleIds.length === 0 || selectedMarkupRules.some((rule) => rule.isPriceBookGroup)}>删除</Button>
                    </Popconfirm>
                  </Space>
                }
              >
              ...
              <Table
                ...
                columns={[
                  { title: priceBookMarkupRules.length ? '价格表/线路' : '代理', dataIndex: 'agentName', width: 180, fixed: 'left' },
                  { title: priceBookMarkupRules.length ? '代理数量' : '规则数量', dataIndex: 'ruleCount', width: 110, render: (value?: number) => `${value ?? 1} 条` },
                  ...
                  { title: '状态', dataIndex: 'enabled', width: 90, render: (enabled: boolean) => <Tag color={enabled ? 'green' : 'default'}>{enabled ? '启用' : '停用'}</Tag> },
                  {
                    ...
                    render: (_, rule) => (
                      <Space size={6}>
                        ...
                        <Button size="small" disabled={rule.isPriceBookGroup} onClick={(event) => { event.stopPropagation(); openEditSpecificMarkupRule(resolveConcreteMarkupRule(rule)); }}>编辑</Button>
                        {rule.enabled ? (
                          <Popconfirm title="确认停用该规则？" okText="确认停用" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={() => resolveAgentRuleIds(rule).then((ids) => Promise.all(ids.map((id) => apiClient.updateAgentMarkupRule(id, { enabled: false })))).then(() => reloadMarkupRules(markupFilters))}>
                            <Button size="small" disabled={rule.isPriceBookGroup} onClick={(event) => event.stopPropagation()}>停用</Button>
                          </Popconfirm>
                        ) : (
                          <Button size="small" disabled={rule.isPriceBookGroup} onClick={(event) => { event.stopPropagation(); enableMarkupRule(rule); }}>启用</Button>
                        )}
                        <Popconfirm title="删除加价规则" description="删除后不可恢复，历史引用记录会保留。" okText="确认删除" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={() => resolveAgentRuleIds(rule).then((ids) => Promise.all(ids.map((id) => apiClient.deleteAgentMarkupRule(id)))).then(() => reloadMarkupRules(markupFilters))}>
                          <Button size="small" danger disabled={rule.isPriceBookGroup} onClick={(event) => event.stopPropagation()}>删除</Button>
                        </Popconfirm>
                      </Space>
                    )
                  }
          ...
          <Card
            ...
            extra={
              <Space>
                ...
                />
                <Button size="small" disabled={selectedPriceBookIds.length !== 1} onClick={openEditPriceBookRemark}>
                  修改备注
                </Button>
                <Popconfirm
                ...
                >
                  <Button size="small" disabled={selectedPriceBookIds.length === 0}>
                    删除价格表
                  </Button>
                </Popconfirm>
              </Space>
          ...
          >
            <Table
              ...
              rowSelection={{
                ...
                onChange: (keys) => setSelectedPriceBookIds(keys.map(String))
              }}
              columns={[
                ...
                { title: '亮崽解析', dataIndex: 'legacyModuleCounts', width: 360, render: (counts: PriceBookSummary['legacyModuleCounts']) => <Text type={counts ? undefined : 'secondary'}>{formatLegacyModuleCounts(counts)}</Text> },
