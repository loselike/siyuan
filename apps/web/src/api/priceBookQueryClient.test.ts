...
describe('PriceBookQueryClient', () => {
  ...
  it('keeps Dubai active display and display-version paths unchanged', async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(1, '/pricing/legacy/dubai-air-sea/display');
    expect(request).toHaveBeenNthCalledWith(2, '/pricing/legacy/dubai-air-sea/display-versions');
  });

  it('keeps the legacy quote metadata query unchanged', async () => {
    const metadata = {} as LegacyPricingMetaResponse;
    const request = vi.fn().mockResolvedValue(metadata) as PriceBookQueryRequest;
    const client = new PriceBookQueryClient(request);

    await expect(client.legacyPricingMeta()).resolves.toBe(metadata);

    expect(request).toHaveBeenCalledWith('/pricing/legacy/quote-meta');
  });
