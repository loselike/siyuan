...
} from './water-receipt-allocation.input.js';
...
describe('parseWaterReceiptUnmatchInput', () => {
  it('preserves valid cancellation ids and reason', () => {
    expect(parseWaterReceiptUnmatchInput({ matchIds: ['match-1', 'match-2'], reason: '重复匹配' })).toEqual({
  ...
  it.each([
    ...
    [{ matchIds: [] }, '没有可撤销的匹配记录'],
    [{ matchIds: [''] }, '没有可撤销的匹配记录'],
    [{ matchIds: ['match-1'], reason: 1 }, '请求数据格式错误']
  ])('rejects malformed cancellation input before repository access: %j', (input, message) => {
    expect(() => parseWaterReceiptUnmatchInput(input)).toThrow(message);
  });
});
