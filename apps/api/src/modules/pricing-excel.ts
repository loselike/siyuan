...
/**
 * Produces a customer-facing transit label without changing the raw pricing
...
function legacySanitizePricingTransitLabel(value?: string | number | null): string | undefined {
  ...
  // A source can state both its main transport promise and a private-address
  // last-mile promise. They describe one customer-facing range, not two
  // unrelated labels: "开船后…30天；私人地址派送时效40天" => "30-40天".
  // Keep this generic to work across sea, rail and air sheets without relying
  // on a supplier-specific column heading.
  ...
  if (mainTransit && addressTransit) {
    const values = [mainTransit[1], addressTransit[1]]
      .flatMap((range) => range.split(/[-~至到]/).map(Number))
  ...
  // Supplier sheets often append compensation limits, fee terms and cargo
  // requirements after a natural-day pickup promise. The promise itself is
  // the customer-facing transit time, so keep only that concise statement.
  const naturalDayPickup = source.match(/(?:交货(?:次日|后)?\s*)?\d+(?:\.\d+)?(?:[-~至到]\d+(?:\.\d+)?)?个自然日(?:内)?提取/i);
  if (naturalDayPickup) {
    return naturalDayPickup[0].replace(/\s+/g, '');
  }

  // Canada sheets frequently write the usable statement after a discount
  // ladder, e.g. "1:300报价… 7-8个工作日提取". Prefer that complete
  // time-before-action form instead of returning the whole source cell.
  const trailingPickup = source.match(/\d+(?:\.\d+)?(?:[-~至到]\d+(?:\.\d+)?)?个?(?:自然|工作)?(?:天|日)(?:左右|内)?(?:提取|派送|交付|签收)/i);
  if (trailingPickup) {
    return trailingPickup[0].replace(/\s+/g, '');
  }

  // Prefer the first complete transit promise before scanning the rest of the
  // cell. Canada sea-rate sheets commonly append volume-discount ladders such
  // as "1:200优惠0.2RMB/KG" without punctuation; those are neither a transit
  // promise nor a channel requirement headline.
  const conciseTransit = source.match(/(?:开船|发车|航程|全程|派送|提取|起飞|交付|签收|时效)\s*(?:后|约|预计|至)?\s*\d+(?:\.\d+)?(?:[-~至到]\d+(?:\.\d+)?)?(?:个)?(?:自然|工作)?(?:天|日)(?:左右|提取|派送)?/i);
  if (conciseTransit) {
    return conciseTransit[0].replace(/\s+/g, '');
  }
  ...
  const segments = source
    ...
    .filter((segment) => {
      if (!TRANSIT_DAY_PATTERN.test(segment.replace(/\s+/g, ''))) return false;
      return /参考时效|全程时效|派送时效|运输时效|航程时效|船期|开船|发车|提取|起飞|飞|派送|交付|签收|时效/i.test(segment)
...
/**
 * Removes a supplier/agent identity from customer-facing channel requirements.
 *
 * The imported price book remains untouched: this is deliberately a display
...
function legacySanitizePricingChannelRequirement(
  ...
  // Remove the source agent first. Some suppliers use a short internal name
  // in the price-book row but spell out the legal entity in the requirement.
  // Longest-first prevents a short alias leaving a partial longer name behind.
  const names = Array.from(new Set(agentNames
  ...
  // Legal Chinese company names are only stripped at a text boundary. This
  // avoids treating ordinary channel requirements as a company merely because
  // they contain a long Chinese phrase.
  text = text
    .replace(/(^|[\n,，;；:：])\s*[\u3400-\u9fff（）()·]{2,40}(?:有限责任公司|股份有限公司|有限公司)/gmu, '$1')
    // Supplier sheets often append an English company spelling after the
    // Chinese legal name, e.g. "Shen zhen PAGO LOGISTICS Co.,Ltd".
    .replace(/\b[A-Z][A-Z0-9 .,&'()\-]{1,80}?(?:\bCO\.?\s*,?\s*(?:LTD\.?|LIMITED)\b|\bLIMITED\b|\bINC\.?\b|\bLLC\b)/gi, '');

  // A price sheet can include an upstream operation centre, warehouse address
  // or contact person in the same cell as the channel requirements. Those
  // details are supplier-facing and must not reach a quote response, including
  // for price books imported before this filter existed.
