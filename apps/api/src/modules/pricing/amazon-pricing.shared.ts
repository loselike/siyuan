import { matchWarehouseCodeRule, type PriceBookRowSummary, type PriceLookupRequest } from '@siyuan/shared';
...
function roundLookupMoney(value: number): number {
  return Math.round(value * 100) / 100;
}

export function calculateLookupChargeableWeight(input: PriceLookupRequest) {
  ...
  const dimensionWeight =
    Number(input.lengthCm ?? 0) > 0 && Number(input.widthCm ?? 0) > 0 && Number(input.heightCm ?? 0) > 0
      ...
      : 0;
  return roundLookupMoney(Math.max(manualWeight, volumeWeight, actualWeight, dimensionWeight));
}

export function createWarehouseLookupProfile(input: PriceLookupRequest) {
...
export function selectPriceRowsForLookup(
